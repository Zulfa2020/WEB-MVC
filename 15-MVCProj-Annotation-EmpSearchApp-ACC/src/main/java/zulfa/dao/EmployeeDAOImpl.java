...
public class EmployeeDAOImpl implements EmployeeDAO {
	private static final String  GET_EMP_DETAILS="(SELECT EMPNO,EMPNAME,SAL,JOB,DEPTNO FROM EMPLOYEE_DETAILS WHERE (EMPNO IS NOT NULL AND EMPNO=?) or  (EMPNAME IS NOT NULL  AND EMPNAME LIKE ?) or (JOB IS NOT NULL  AND JOB LIKE ?)  or   (SAL IS NOT NULL AND SAL >=?))"; 
	...
	public List<EmployeeResultBO> find(EmployeeBO bo) {
		...
		listRBO=jt.query(GET_EMP_DETAILS,
				                             ...
				                             bo.getEmpno(),bo.getEmpname(),bo.getJob(),bo.getSal());
