...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	private  static final String GET_EMP_BY_NO="SELECT EMPNO,EMPNAME,JOB,SAL,DEPTNO FROM EMPLOYEE_DETAILS WHERE EMPNO=?";
	private  static final String UPDATE_EMP_BY_NO="UPDATE EMPLOYEE_DETAILS SET EMPNAME=?,JOB=?,SAL=?,DEPTNO=? WHERE EMPNO=?";
	private  static final String DELETE_EMP_BY_NO="DELETE  FROM EMPLOYEE_DETAILS WHERE EMPNO=?";
	private static final String  INSER_INTO_TABLE="INSERT INTO EMPLOYEE_DETAILS VALUES(?,?,?,?,?)" ;
	...
	public int insert(EmployeeBO bo) {
		int count=0;
		count=jt.update(INSER_INTO_TABLE,bo.getEmpNo(),bo.getEname(),bo.getJob(),bo.getSal(),bo.getDeptNo());
